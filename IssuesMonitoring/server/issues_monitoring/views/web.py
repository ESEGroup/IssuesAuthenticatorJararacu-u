...
@app.route('/registerentry', methods=['POST'])
def registerentry():
	dict = {'user_id': request.form.get('user_id') or '', 'lab_id': request.form.get('lab_id') or '',
	'event': request.form.get('event') or '', 'epoch': request.form.get('epoch') or ''}
	controllers.registrar_presenca([dict])
	return jsonify({'success': True})

@app.route('/getalllabs')
def getalllabs():
	labs_info = controllers.obter_informacoes_labs()
